init python:
    class GridManager:
        ...
        def initialize_grid(self):
            ...
            self.apply_chain_locks() 

        def apply_chain_locks(self):
            # Only activate chains if the game level is 3 or 4.
            # (Assume that game.level is now properly set in your GameManager.)
            if game.level < 3:
                return
            import random
            for icon in self.icons:
                if icon is not None:
                    # With a 20% chance, lock the tile.
                    if random.random() < 0.20:
                        icon.chain_locked = True
                        # Create the overlay.
        ...
        def shift_icons(self, mouse_event):
            # Shifting icons (if any gaps exist).
