init python:
    ...
    def check_target_objective():
        if inventory.get("brick", 0) >= 2: # Need to be integrated 
            renpy.show_screen("Building_Screen")
...
screen Inventory_UI:
    frame:
        ...
        padding (10, 10, 10, 10)
        vbox:
            text "Inventory" size 30 color "#000000"
            for material, count in inventory.items():
                hbox:
                    spacing 10
                    add "Icons/{}.png".format(material) zoom 0.1 yalign 0.5
...
label hut_sublevel_1:
    "Hut Sublevel 1 chosen."
    return
label hut_sublevel_2:
    "Hut Sublevel 2 chosen."
    return
label hut_sublevel_3:
    "Hut Sublevel 3 chosen."
    return
label hut_sublevel_4:
    "Hut Sublevel 4 chosen."
    return
...
# Labels to call the sublevel screens
